...
void Orderbook::PruneGoodForDayOrders()
{
    ...
    while (true)
    {
        ...
        OrderIds orderIds;
        {
            std::scoped_lock orderdLock{ ordersMutex_ };
...
bool Orderbook::CanFullyFill(Side side, Price price, Quantity quantity) const
{
    ...
    for (const auto& [levelPrice, levelData] : data_)
    {
        ...
        if ((side == Side::Buy && levelPrice > price) ||
            ...
            continue;
        if (levelData.quantity_ <= quantity)
            return true;
...
Trades Orderbook::MatchOrders()
{
    ...
    while (true)
    {
        ...
        {
            auto bid = bestBidOrders.front();
            auto ask = bestAskOrders.front();
            ...
            OnOrderMatched(bestBidPrice, quantity, bid->IsFilled());
            OnOrderMatched(bestAskPrice, quantity, ask->IsFilled());
        }

        if (bestBidOrders.empty())
        {
            bids_.erase(bestBidPrice);
            data_.erase(bestBidPrice);
        }
        if (bestAskOrders.empty())
        {
            asks_.erase(bestAskPrice);
            data_.erase(bestAskPrice);
        }
    }
    if (!bids_.empty())
    {
...
Orderbook::Orderbook() : ordersPrunceThread_{ [this] { PruneGoodForDayOrders(); } } { }

Orderbook::~Orderbook()
{
	shutdown_.store(true, std::memory_order_release);
    shutdownConditionVariable_.notify_one();
    ordersPrunceThread_.join();
}
